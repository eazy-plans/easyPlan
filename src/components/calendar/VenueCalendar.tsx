...
export function VenueCalendar({ venues, initialEvents, userId, role }: VenueCalendarProps) {
  ...
  // A day only reads as solidly "booked" (gray) if it has an approved event
  // that ISN'T flagged cancellation_requested_at - a day whose only approved
  // events are all pending-cancellation is available to a new client, so it
  // gets the lighter warning treatment instead (see pendingDates below).
