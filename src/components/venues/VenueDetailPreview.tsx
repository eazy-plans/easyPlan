...
interface VenueDetailPreviewProps {
  venue: VenueRow;
    ...
    <div className="flex flex-col lg:flex-row gap-6">
      ...
      <div className="lg:w-[380px] shrink-0 space-y-4">
        ...
        {venue.description_long && (
          <p className="text-sm text-muted-foreground whitespace-pre-wrap">{venue.description_long}</p>
        )}
      </div>
            ...
            {PRICE_KEYS.map(({ key, label }) => {
              ...
              return (
                <div key={key} className={`flex justify-between text-sm px-3 py-2 rounded-md ${
                  eventType && label === EVENT_TYPE_LABELS[eventType] ? "bg-primary/10 font-medium" : "bg-muted"
                }`}>
                  <span>{label}</span>
